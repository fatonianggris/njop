...
class Auth extends MX_Controller {
    ...
    public function index() {
        if ($this->session->userdata('sias-officer') == TRUE) {
            redirect('officer/dashboard');
        }
        $data['title'] = 'Login SiNJOP | Acitya Technology ';
        $data['page'] = $this->AuthModel->get_page();
        $this->load->view('officer_auth_login', $data);
    }
            ...
            if ($cek_user) {
                ...
                if (password_verify($data['password'], $cek_user[0]->password)) {
                    ...
                    if ($this->googleCaptachStore($recaptchaResponse, $userIp) == 1) {
                        ...
                        $this->session->set_userdata('sias-officer', $data_user);
                        redirect('officer/dashboard');
                    } else {
    ...
    public function logout() {
        ...
        $this->session->unset_userdata('sias-officer');
